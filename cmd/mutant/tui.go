...
// virusRow tracks per-virus mutation result counts for the TUI table.
...
// tuiModel is the Bubble Tea model for the --mode=table live progress display.
...
// progressMsg wraps MutationProgress as a Bubble Tea message.
// runDoneMsg signals that the mutation run goroutine has finished.
type (
...
func (m *tuiModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case progressMsg:
		m.applyProgress(mutant.MutationProgress(msg))
...
func (m *tuiModel) applyProgress(p mutant.MutationProgress) {
	m.phase = p.Phase
	if p.Message != "" {
	...
	m.recordVirusResult(p.Mutation.Mutator, p.Result.Status)
}

func (m *tuiModel) recordVirusResult(virus string, status mutant.Status) {
	row, ok := m.virusStats[virus]
	if !ok {
		row = &virusRow{}
		m.virusStats[virus] = row
		m.virusOrder = append(m.virusOrder, virus)
		slices.Sort(m.virusOrder)
	}
...
func (m *tuiModel) renderHeader(b *strings.Builder) {
	phase := m.phaseMsg
	if phase == "" {
		phase = "Initializing..."
	}
...
func (m *tuiModel) renderVirusTable(b *strings.Builder) {
...
func (m *tuiModel) renderProgress(b *strings.Builder) {
	if m.total == 0 {
		return
	}
...
func (m *tuiModel) renderSummary(b *strings.Builder) {
	b.WriteString("\n")
	...
	m.renderSurvivors(b, survived)
}

func (m *tuiModel) renderSurvivors(b *strings.Builder, survived int) {
	b.WriteString("\n")
